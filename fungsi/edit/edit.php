...
// =================================================================
//              BAGIAN 1: BLOK KEAMANAN (CUKUP SATU INI)
// =================================================================
...
if (empty($_SESSION['admin'])) {
    ...
    // Ambil peran (role) pengguna dan aksi yang sedang dilakukan
    $role = $_SESSION['admin']['role'];
    $action = !empty($_GET) ? key($_GET) : ''; // Mengambil aksi dari URL, misal: 'pengaturan', 'kategori', 'jual'

    // Definisikan aksi apa saja yang diizinkan untuk 'kasir' di file ini
    $kasir_allowed_actions = ['jual', 'cari_barang'];

    // ATURAN UNTUK PEMILIK: Tolak semua aksi di file edit ini.
    if ($role == 'pemilik') {
        echo '<script>alert("Akses Ditolak! Pemilik hanya dapat melihat laporan.");window.history.back();</script>';
        exit;
    }

    // ATURAN UNTUK KASIR: Hanya izinkan aksi yang ada di daftar $kasir_allowed_actions
    if ($role == 'kasir' && !in_array($action, $kasir_allowed_actions)) {
        echo '<script>alert("Akses Ditolak! Kasir tidak memiliki izin untuk melakukan aksi ini.");window.history.back();</script>';
        exit;
    }
    // =================================================================
//                  AKHIR DARI BLOK KEAMANAN
// =================================================================


    // =================================================================
//         BAGIAN 2: KODE ASLI FILE EDIT.PHP ANDA DIMULAI DARI SINI
// =================================================================
    require '../../config.php';

    if (!empty($_GET['pengaturan'])) {
        // ... kode untuk edit pengaturan ...
    }

    if (!empty($_GET['kategori'])) {
        // ... kode untuk edit kategori ...
    }

    // ... dan seterusnya, seluruh sisa kode asli file edit.php Anda ...
